...
int		g_iVOPrefixOffset = 0;

public void OnPluginStart()
{
	Handle gameConf = LoadGameConfigFile("agentvoice.games");
	if (gameConf == null)
	{
	...
	PrepSDKCall_SetFromConf(gameConf, SDKConf_Signature, "CUtlString::Set");
	PrepSDKCall_AddParameter(SDKType_String, SDKPass_Pointer);

	if (!(g_SDKCUtlStringSet = EndPrepSDKCall()))
	{
		SetFailState("Failed to load SDKCall \"CUtlString::Set\". Update your gamedata.");
		return;
	}

	g_iVOPrefixOffset = GameConfGetOffset(gameConf, "vo_prefix");
	if (!g_iVOPrefixOffset)
	{
		SetFailState("Unable to get offset of \"vo_prefix\". Update your gamedata.");
		return;
	}
...
void SetClientVoPrefix(int client, const char[] voPrefix)
{
	if(IsPlayer(client))
	{
		SDKCall(g_SDKCUtlStringSet, GetEntityAddress(client) + view_as<Address>(g_iVOPrefixOffset), voPrefix);
	}
}
